...
static guint32 WINAPI start_wrapper_internal(void *data)
{
	 ...
	 */
	mono_thread_new_init (tid, &tid, (gpointer)start_func);
...
void ves_icall_System_Threading_Thread_Interrupt_internal (MonoThread *this_obj)
{
	...
	should_throw = current != thread && (thread->state & ThreadState_WaitSleepJoin);
...
{
	MonoInternalThread *thread = (MonoInternalThread *)ud;
...
ref_stack_push (RefStack *rs, gpointer ptr)
{
	...
	if (rs->bottom >= rs->allocated) {
		rs->refs = (gpointer *)g_realloc (rs->refs, rs->allocated * 2 * sizeof (gpointer) + 1);
		rs->allocated <<= 1;
...
mono_alloc_static_data (gpointer **static_data_ptr, guint32 offset, gboolean threadlocal)
{
	...
	if (!static_data) {
		...
		static_data = (gpointer *)mono_gc_alloc_fixed (static_data_size [0], threadlocal ? tls_desc : ctx_desc,
...
mono_alloc_static_data_slot (StaticDataInfo *static_data, guint32 size, guint32 align)
{
	...
	guint32 offset = MAKE_SPECIAL_STATIC_OFFSET ((guint32)static_data->idx, (guint32)static_data->offset, 0);
...
thread_adjust_static_data (MonoInternalThread *thread)
{
	...
	if (thread_static_info.offset || thread_static_info.idx > 0) {
		...
		guint32 offset = MAKE_SPECIAL_STATIC_OFFSET ((guint32)thread_static_info.idx, (guint32)thread_static_info.offset, 0);
...
context_adjust_static_data (MonoAppContext *ctx)
{
	if (context_static_info.offset || context_static_info.idx > 0) {
		guint32 offset = MAKE_SPECIAL_STATIC_OFFSET ((guint32)context_static_info.idx, (guint32)context_static_info.offset, 0);
