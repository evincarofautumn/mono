...
mono_class_compute_gc_descriptor (MonoClass *klass)
{
	...
	if (!gcj_inited) {
		...
		mono_register_jit_icall ((gpointer)mono_object_new_fast, "mono_object_new_fast", mono_create_icall_signature ("object ptr"), FALSE);
		mono_register_jit_icall ((gpointer)mono_string_alloc, "mono_string_alloc", mono_create_icall_signature ("object int"), FALSE);
	...
	if (klass == mono_defaults.string_class) {
		...
		/*
		if (klass->gc_descr == MONO_GC_DESCRIPTOR_NULL)
			g_print ("disabling typed alloc (%d) for %s.%s\n", max_set, klass->name_space, klass->name);
		*/
		/*printf ("new descriptor: %p 0x%x for %s.%s\n", klass->gc_descr, bitmap [0], klass->name_space, klass->name);*/
		if (bitmap != default_bitmap)
			g_free (bitmap);
	}
}
											...
											MonoMethod *method, gpointer code)
{
	...
	if (++gvc->count == THUNK_THRESHOLD) {
		gpointer *old_thunk = (gpointer *)*vtable_slot;
...
mono_class_create_runtime_vtable (MonoDomain *domain, MonoClass *klass, gboolean raise_on_error)
{
	...
	if (class_size) {
		/* we store the static field pointer at the end of the vtable: vt->vtable [klass->vtable_size] */
	...
	//printf ("Initializing VT for klass %s (interface_offsets_count = %d)\n",
...
create_remote_class_key (MonoRemoteClass *remote_class, MonoClass *extra_class)
{
	...
	if (remote_class == NULL) {
		...
		if (extra_class != NULL && (extra_class->flags & TYPE_ATTRIBUTE_INTERFACE)) {
			...
			key = (gpointer *)g_malloc (sizeof(gpointer) * (remote_class->interface_count + 2));
...
handle_enum:
	...
	case MONO_TYPE_OBJECT:
	case MONO_TYPE_ARRAY:
		mono_gc_wbarrier_generic_store (dest, (MonoObject *)(deref_pointer ? *(gpointer*)value : value));
		return;
	case MONO_TYPE_FNPTR:
	case MONO_TYPE_PTR: {
		gpointer *p = (gpointer*)dest;
...
mono_runtime_exec_managed_code (MonoDomain *domain,
				...
				gpointer main_args)
{
	mono_thread_create (domain, (gpointer)main_func, main_args);
			   ...
			   MonoObject **exc)
{
	...
	if (NULL != params) {
		pa = (gpointer *)alloca (sizeof (gpointer) * mono_array_length (params));
...
#endif
			if (method->klass->valuetype)
				o = mono_object_unbox ((MonoObject *)obj);
			else
				o = obj;
...
mono_class_get_allocation_ftn (MonoVTable *vtable, gboolean for_box, gboolean *pass_size_in_words)
{
	...
	*pass_size_in_words = FALSE;
	...
	if (mono_class_has_finalizer (vtable->klass) || mono_class_is_marshalbyref (vtable->klass) || (mono_profiler_get_events () & MONO_PROFILE_ALLOCATIONS))
		return (void *)mono_object_new_specific;

	if (vtable->gc_descr != MONO_GC_DESCRIPTOR_NULL) {

		return (void *)mono_object_new_fast;
...
mono_array_clone_in_domain (MonoDomain *domain, MonoArray *array)
{
	...
	sizes = (uintptr_t *)alloca (klass->rank * sizeof(uintptr_t) * 2);
	size = mono_array_element_size (klass);
