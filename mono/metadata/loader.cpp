		      ...
		      MonoGenericContext *context, MonoError *error)
{
	...
	class_token = cols [MONO_MEMBERREF_CLASS] & MONO_MEMBERREF_PARENT_MASK;

	fname = mono_metadata_string_heap (image, cols [MONO_MEMBERREF_NAME]);

	if (!mono_verifier_verify_memberref_field_signature (image, cols [MONO_MEMBERREF_SIGNATURE], NULL)) {
		mono_error_set_bad_image (error, image, "Bad field '%s' signature 0x%08x", class_token, token);
		return NULL;
	}
	...
	case MONO_MEMBERREF_PARENT_TYPESPEC:
		...
		break;
	default:
		mono_error_set_bad_image (error, image, "Bad field field '%s' signature 0x%08x", class_token, token);
	}
	...
	if (*ptr++ != 0x6) {
		mono_error_set_field_load (error, klass, fname, "Bad field signature class token %08x field name %s token %08x", class_token, fname, token);
		return NULL;
	}
	...
	if (image_is_dynamic (image)) {
		...
		if (!sig) {
			if (!mono_verifier_verify_memberref_method_signature (image, sig_idx, NULL)) {
				guint32 class_token = cols [MONO_MEMBERREF_CLASS] & MONO_MEMBERREF_PARENT_MASK;
				const char *fname = mono_metadata_string_heap (image, cols [MONO_MEMBERREF_NAME]);

				//FIXME include the verification error
				mono_error_set_bad_image (error, image, "Bad method signature class token 0x%08x field name %s token 0x%08x", class_token, fname, token);
				return NULL;
			}
		...
		if (!mono_verifier_is_sig_compatible (image, method, sig)) {
			...
			mono_error_set_bad_image (error, image, "Incompatible method signature class token 0x%08x field name %s token 0x%08x", class_token, fname, token);
			return NULL;
		}
	}
		       ...
		       gboolean *used_context, MonoError *error)
{
	...
	MonoTableInfo *tables = image->tables;
	guint32 cols[6];
	guint32 nindex, class_token, sig_idx;
	const char *mname;
	MonoMethodSignature *sig;
	const char *ptr;
	 ...
	 */
	if (used_context)
		*used_context = class_token == MONO_MEMBERREF_PARENT_TYPESPEC;
	...
	default:
		mono_error_set_bad_image (error, image, "Memberref parent unknown: class: %d, index %d", class_token, nindex);
		goto fail;
	}
