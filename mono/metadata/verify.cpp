 ...
 */
static gboolean
mono_type_is_value_type (MonoType *type, const char *ns, const char *name)
{
	return type->type == MONO_TYPE_VALUETYPE &&
		!strcmp (ns, type->data.klass->name_space) &&
 ...
 */
static void
do_invoke_method (VerifyContext *ctx, int method_token, gboolean is_virtual)
{
	MonoError error;
	int param_count, i;
	...
	if (!(method = verifier_load_method (ctx, method_token, is_virtual ? "callvirt" : "call")))
		return;
	...
	for (i = sig->param_count - 1; i >= 0; --i) {
		...
		if (stack_slot_is_managed_mutability_pointer (value))
			CODE_NOT_VERIFIABLE (ctx, g_strdup_printf ("Cannot use a readonly pointer as argument of %s at 0x%04x", is_virtual ? "callvirt" : "call",  ctx->ip_offset));

		if ((ctx->prefix_set & PREFIX_TAIL) && stack_slot_is_managed_pointer (value)) {
			ADD_VERIFY_ERROR (ctx, g_strdup_printf ("Cannot  pass a byref argument to a tail %s at 0x%04x", is_virtual ? "callvirt" : "call",  ctx->ip_offset));
			return;
		}
	}
	...
	if (sig->hasthis) {
		...
		if (constrained && is_virtual) {
			...
			if (!is_virtual && mono_class_from_mono_type (value->type)->valuetype && !method->klass->valuetype && !stack_slot_is_boxed_value (value))
				CODE_NOT_VERIFIABLE (ctx, g_strdup_printf ("Cannot call a valuetype baseclass at 0x%04x", ctx->ip_offset));
	
			if (is_virtual && mono_class_from_mono_type (value->type)->valuetype && !stack_slot_is_boxed_value (value))
...
static void
do_load_function_ptr (VerifyContext *ctx, guint32 token, gboolean is_virtual)
{
	ILStackDesc *top;
	MonoMethod *method;

	if (is_virtual && !check_underflow (ctx, 1))
		return;

	if (!is_virtual && !check_overflow (ctx))
		return;
	...
	if (ctx->method->wrapper_type != MONO_WRAPPER_NONE) {
		...
		if (!(method = verifier_load_method (ctx, token, is_virtual ? "ldvirtfrn" : "ldftn")))
			return;
	}
