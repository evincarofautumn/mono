		...
		for (i = 0; i < marray->num_monitors; ++i) {
			...
			if (mon->data == NULL) {
				...
				if (!monitor_is_on_freelist ((MonoThreadsSync *)mon->data)) {
					MonoObject *holder = (MonoObject *)mono_gchandle_get_target ((guint32)(size_t)mon->data);
...
mon_new (gsize id)
{
	...
	if (!monitor_freelist) {
		...
		for (marray = monitor_allocated; marray; marray = marray->next) {
			for (i = 0; i < marray->num_monitors; ++i) {
				if (mono_gchandle_get_target ((guint32)(size_t)marray->monitors [i].data) == NULL) {
					result = &marray->monitors [i];
					if (result->wait_list) {
						/* Orphaned events left by aborted threads */
						while (result->wait_list) {
							LOCK_DEBUG (g_message (G_GNUC_PRETTY_FUNCTION ": (%d): Closing orphaned event %d", mono_thread_info_get_small_id (), result->wait_list->data));
							CloseHandle (result->wait_list->data);
							result->wait_list = g_slist_remove (result->wait_list, result->wait_list->data);
						}
					}
					mono_gchandle_free ((guint32)(size_t)result->data);
					result->data = monitor_freelist;
					monitor_freelist = result;
				}
			}
		...
		if (!monitor_freelist) {
			...
			marray = (MonitorArray *)g_malloc0 (sizeof (MonoArray) + array_size * sizeof (MonoThreadsSync));
	...
	result = monitor_freelist;
	monitor_freelist = (MonoThreadsSync *)result->data;

	result->status = mon_status_set_owner (0, id);
	result->status = mon_status_init_entry_count (result->status);
	result->nest = 1;
	result->data = NULL;
...
{
	mono_monitor_allocator_lock ();
	mono_gchandle_free ((guint32)(size_t)mon->data);
	mon_finalize (mon);
	mono_monitor_allocator_unlock ();
}
...
mono_monitor_get_object_monitor_gchandle (MonoObject *object)
{
	...
	if (lock_word_is_inflated (lw)) {
		...
		return (guint32)(size_t)mon->data;
	}
	return 0;
}
