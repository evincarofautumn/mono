...
static void
scan_for_string_callback (char *start, size_t size, void *data)
{
	MonoObject *obj = (MonoObject *)start;
	MonoString *str;
	char *search = (char *)data;

	if (sgen_obj_get_descriptor ((char *)obj) != SGEN_DESC_STRING)
		return;
...
sgen_scan_for_string (char *search) {
	sgen_scan_area_with_callback (nursery_section->data, nursery_section->end_data, scan_for_string_callback, (void *)search, FALSE);
