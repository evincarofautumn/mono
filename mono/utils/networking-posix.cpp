...
mono_get_local_interfaces (int family, int *interface_count)
{
	...
	*interface_count = 0;
	...
	ifc.ifc_buf = (caddr_t)g_malloc (IFCONF_BUFF_SIZE); /* We can't have such huge buffers on the stack. */
	...
	result_ptr = (char *)g_malloc (if_count * mono_address_size_for_family (family));;
	result = (char *)result_ptr;
	FOREACH_IFR (ifr, ifc) {
		if (ifr->ifr_name [0] == '\0')
			continue;
