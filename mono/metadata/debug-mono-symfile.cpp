	...
	if (raw_contents != NULL) {
		...
		if ((f = mono_file_map_open (symfile->filename))) {
			...
			if (symfile->raw_contents_size == 0) {
				...
				symfile->raw_contents = (const uint8_t *)mono_file_map (symfile->raw_contents_size, MONO_MMAP_READ|MONO_MMAP_PRIVATE, mono_file_map_fd (f), 0, &symfile->raw_contents_handle);
			}
