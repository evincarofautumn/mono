...
static void mono_generic_class_setup_parent (MonoClass *klass, MonoClass *gclass);
...
MonoClass*
mono_class_inflate_generic_class_checked (MonoClass *gclass, MonoGenericContext *context, MonoError *error)
{
	MonoClass *res;
	MonoType *inflated;

	inflated = mono_class_inflate_generic_type_checked (&gclass->byval_arg, context, error);
	if (!mono_error_ok (error))
		return NULL;
 ...
 *
 *   Inflate the class GCLASS with CONTEXT.
 */
MonoClass*
mono_class_inflate_generic_class (MonoClass *gclass, MonoGenericContext *context)
{
	MonoError error;
	MonoClass *res;

	res = mono_class_inflate_generic_class_checked (gclass, context, &error);
	g_assert (mono_error_ok (&error)); /*FIXME proper error handling*/
...
mono_class_find_enum_basetype (MonoClass *klass, MonoError *error)
{
	...
	if (klass->generic_container)
		...
		container = gclass->generic_container;
		g_assert (container);
	}
...
mono_class_setup_basic_field_info (MonoClass *klass)
{
	...
	if (klass->generic_class && image_is_dynamic (klass->generic_class->container_class->image) && !klass->generic_class->container_class->wastypebuilder) {
		 ...
		 * the fields of this class using the fields of gclass, since gclass is not
...
mono_class_setup_methods (MonoClass *klass)
{
	...
	if (klass->generic_class) {
		...
		if (!gclass->exception_type)
			mono_class_setup_methods (gclass);
		if (gclass->exception_type) {
		...
		for (i = 0; i < count; i++) {
			methods [i] = mono_class_inflate_generic_method_full_checked (
				gclass->methods [i], klass, mono_class_get_context (klass), &error);
			if (!mono_error_ok (&error)) {
				char *method = mono_method_full_name (gclass->methods [i], TRUE);
...
mono_class_get_method_by_index (MonoClass *klass, int index)
{
	...
	if (klass->generic_class && !klass->methods) {
		...
		m = mono_class_inflate_generic_method_full_checked (
				gclass->methods [index], klass, mono_class_get_context (klass), &error);
...
mono_class_get_inflated_method (MonoClass *klass, MonoMethod *method)
{
	...
	g_assert (method->klass == gclass);

	mono_class_setup_methods (gclass);
	g_assert (!gclass->exception_type); /*FIXME do proper error handling*/

	for (i = 0; i < gclass->method.count; ++i) {
		if (gclass->methods [i] == method) {
			if (klass->methods) {
				return klass->methods [i];
			} else {
				MonoError error;
				MonoMethod *result = mono_class_inflate_generic_method_full_checked (gclass->methods [i], klass, mono_class_get_context (klass), &error);
...
mono_class_get_vtable_entry (MonoClass *klass, int offset)
{
	...
	if (klass->generic_class) {
		MonoError error;
		MonoClass *gclass = klass->generic_class->container_class;
		mono_class_setup_vtable (gclass);
		m = gclass->vtable [offset];
 ...
 *
 * This method can fail the klass.
 */
static void
mono_class_setup_properties (MonoClass *klass)
{
	...
	if (klass->generic_class) {
		...
		mono_class_init (gclass);
		mono_class_setup_properties (gclass);
		if (gclass->exception_type) {
			mono_class_set_failure (klass, MONO_EXCEPTION_TYPE_LOAD, g_strdup ("Generic type definition failed to load"));
			return;
		}

		properties = mono_class_new0 (klass, MonoProperty, gclass->ext->property.count + 1);

		for (i = 0; i < gclass->ext->property.count; i++) {
			MonoError error;
			MonoProperty *prop = &properties [i];

			*prop = gclass->ext->properties [i];
		...
		first = gclass->ext->property.first;
		count = gclass->ext->property.count;
	} else {
...
mono_class_setup_events (MonoClass *klass)
{
	...
	if (klass->generic_class) {
		...
		mono_class_setup_events (gclass);
		if (gclass->exception_type) {
			mono_class_set_failure (klass, MONO_EXCEPTION_TYPE_LOAD, g_strdup ("Generic type definition failed to load"));
			return;
		}

		first = gclass->ext->event.first;
		count = gclass->ext->event.count;
		...
		for (i = 0; i < count; i++) {
			...
			MonoEvent *gevent = &gclass->ext->events [i];
 ...
 * @klass: interface
 *
 * Assign a unique integer ID to the interface represented by @klass.
...
collect_implemented_interfaces_aux (MonoClass *klass, GPtrArray **res, MonoError *error)
{
	...
	for (i = 0; i < klass->interface_count; i++) {
		...
		if (ic->exception_type) {
			mono_error_set_type_load_class (error, ic, "Error Loading klass");
			return;
		}
...
print_implemented_interfaces (MonoClass *klass) {
	...
	printf ("Packed interface table for klass %s has size %d\n", name, klass->interface_offsets_count);
	g_free (name);
	...
	printf ("\n");
	while (klass != NULL) {
		printf ("[LEVEL %d] Implemented interfaces by klass %s:\n", ancestor_level, klass->name);
...
static void
fill_valuetype_array_derived_types (MonoClass **valuetype_types, MonoClass *eklass, int rank)
{
	valuetype_types [0] = eklass;
	if (eklass == mono_defaults.int16_class)
		valuetype_types [1] = mono_defaults.uint16_class;
	else if (eklass == mono_defaults.uint16_class)
		valuetype_types [1] = mono_defaults.int16_class;
	else if (eklass == mono_defaults.int32_class)
		valuetype_types [1] = mono_defaults.uint32_class;
	else if (eklass == mono_defaults.uint32_class)
		valuetype_types [1] = mono_defaults.int32_class;
	else if (eklass == mono_defaults.int64_class)
		valuetype_types [1] = mono_defaults.uint64_class;
	else if (eklass == mono_defaults.uint64_class)
		valuetype_types [1] = mono_defaults.int64_class;
	else if (eklass == mono_defaults.byte_class)
		valuetype_types [1] = mono_defaults.sbyte_class;
	else if (eklass == mono_defaults.sbyte_class)
		valuetype_types [1] = mono_defaults.byte_class;
	else if (eklass->enumtype && mono_class_enum_basetype (eklass))
		valuetype_types [1] = mono_class_from_mono_type (mono_class_enum_basetype (eklass));
}
...
{
	MonoClass *eklass = klass->element_class;
	...
	eklass_is_valuetype = FALSE;
	original_rank = eklass->rank;
	if (klass->byval_arg.type != MONO_TYPE_SZARRAY) {
		if (klass->generic_class && klass->nested_in == mono_defaults.array_class && strcmp (klass->name, "InternalEnumerator`1") == 0)	 {
			 ...
			 */
			eklass = mono_class_from_mono_type (klass->generic_class->context.class_inst->type_argv [0]);
			original_rank = eklass->rank;
			if (!eklass->rank)
				eklass = eklass->element_class;
	 ...
	 */
	all_interfaces = eklass->rank && eklass->element_class->rank? FALSE: TRUE;
	 ...
	 * on the element klass). For net 4.5, we also need to implement IReadOnlyList`1/IReadOnlyCollection`1.
	...
	if (eklass->valuetype) {
		...
		fill_valuetype_array_derived_types (valuetype_types, eklass, original_rank);
		...
		eklass_is_valuetype = TRUE;
	} else {
		int j;
		int idepth = eklass->idepth;
		if (!internal_enumerator)
			idepth--;
		nifaces = generic_ireadonlylist_class ? 2 : 3;

		// FIXME: This doesn't seem to work/required for generic params
		if (!(eklass->this_arg.type == MONO_TYPE_VAR || eklass->this_arg.type == MONO_TYPE_MVAR || (image_is_dynamic (eklass->image) && !eklass->wastypebuilder)))
			mono_class_setup_interface_offsets (eklass);

		interface_count = all_interfaces? eklass->interface_offsets_count: eklass->interface_count;
		/* we add object for interfaces and the supertypes for the other
		 * types. The last of the supertypes is the element klass itself which we
		 * already created the explicit interfaces for (so we include it for IEnumerator
		 * and exclude it for arrays).
		 */
		if (MONO_CLASS_IS_INTERFACE (eklass))
			interface_count++;
		else
			interface_count += idepth;
		if (eklass->rank && eklass->element_class->valuetype) {
			fill_valuetype_array_derived_types (valuetype_types, eklass->element_class, original_rank);
			if (valuetype_types [1])
				++interface_count;
		}
		...
		if (internal_enumerator) {
			real_count += (MONO_CLASS_IS_INTERFACE (eklass) ? 1 : idepth) + eklass->interface_offsets_count;
			if (valuetype_types [1])
				++real_count;
		}
		interfaces = (MonoClass **)g_malloc0 (sizeof (MonoClass*) * real_count);
		if (MONO_CLASS_IS_INTERFACE (eklass)) {
			...
			j = 0;
			for (i = 0; i < idepth; i++) {
				mono_class_init (eklass->supertypes [i]);
				interfaces [j] = eklass->supertypes [i];
				j += nifaces;
			}
		}
		if (all_interfaces) {
			for (i = 0; i < eklass->interface_offsets_count; i++) {
				interfaces [j] = eklass->interfaces_packed [i];
				j += nifaces;
			}
		} else {
			for (i = 0; i < eklass->interface_count; i++) {
				interfaces [j] = eklass->interfaces [i];
				j += nifaces;
			}
		}
		if (valuetype_types [1]) {
	...
	if (internal_enumerator) {
		...
		j = interface_count;
		if (!eklass_is_valuetype) {
			if (MONO_CLASS_IS_INTERFACE (eklass)) {
				interfaces [j] = inflate_class_one_arg (generic_ienumerator_class, mono_defaults.object_class);
				j ++;
			} else {
				for (i = 0; i < eklass->idepth; i++) {
					interfaces [j] = inflate_class_one_arg (generic_ienumerator_class, eklass->supertypes [i]);
					j ++;
				}
			}
			for (i = 0; i < eklass->interface_offsets_count; i++) {
				interfaces [j] = inflate_class_one_arg (generic_ienumerator_class, eklass->interfaces_packed [i]);
				j ++;
			}
		} else {
 ...
 * mono_class_setup_vtable:
 *
 *   Creates the generic vtable of KLASS.
...
check_interface_method_override (MonoClass *klass, MonoMethod *im, MonoMethod *cm, gboolean require_newslot, gboolean interface_is_explicitly_implemented_by_class, gboolean slot_is_empty)
{
	...
	if (strcmp (im->name, cm->name) == 0) {
		...
		if (strstr (subname, ic_name) != subname) {
			TRACE_INTERFACE_VTABLE (printf ("[ACTUAL KLASS NAME CHECK FAILED]"));
			return FALSE;
		}
...
print_vtable_full (MonoClass *klass, MonoMethod** vtable, int size, int first_non_interface_slot, const char *message, gboolean print_interfaces) {
	...
	printf ("*** Vtable for klass '%s' at \"%s\" (size %d)\n", full_name, message, size);
	
	if (print_interfaces) {
		print_implemented_interfaces (klass);
		printf ("* Interfaces for klass '%s' done.\nStarting vtable (size %d):\n", full_name, size);
	}
...
mono_class_verify_vtable (MonoClass *klass)
{
	...
	printf ("*** Verifying VTable of klass '%s' \n", full_name);
	g_free (full_name);
	full_name = NULL;
...
print_unimplemented_interface_method_info (MonoClass *klass, MonoClass *ic, MonoMethod *im, int im_slot, MonoMethod **overrides, int onum) {
	...
	mono_trace_warning (MONO_TRACE_TYPE, "no implementation for interface method %s::%s(%s) in klass %s\n",
	...
	if (klass->exception_type) {
		...
		mono_trace_warning (MONO_TRACE_TYPE, "KLASS %s failed to resolve methods\n", name);
		g_free (name);
		return;
	}
...
verify_class_overrides (MonoClass *klass, MonoMethod **overrides, int onum)
{
	...
	for (i = 0; i < onum; ++i) {
		...
		if (mono_class_get_generic_type_definition (body->klass) != mono_class_get_generic_type_definition (klass)) {
			mono_class_set_failure (klass, MONO_EXCEPTION_TYPE_LOAD, g_strdup ("Method belongs to a different klass than the declared one"));
			return FALSE;
		}
		...
		if (!mono_class_is_assignable_from_slow (decl->klass, klass)) {
			mono_class_set_failure (klass, MONO_EXCEPTION_TYPE_LOAD, g_strdup ("Method overrides a klass or interface that extended or implemented by this type"));
			return FALSE;
		}
...
mono_class_setup_vtable_general (MonoClass *klass, MonoMethod **overrides, int onum, GList *in_setup)
{
	...
	if (klass->generic_class) {
		...
		mono_class_setup_vtable_full (gclass, in_setup);
		if (gclass->exception_type != MONO_EXCEPTION_NONE) {
			mono_class_set_failure (klass, MONO_EXCEPTION_TYPE_LOAD, NULL);
			return;
		}

		tmp = (MonoMethod **)mono_class_alloc0 (klass, sizeof (gpointer) * gclass->vtable_size);
		klass->vtable_size = gclass->vtable_size;
		for (i = 0; i < gclass->vtable_size; ++i)
			if (gclass->vtable [i]) {
				MonoMethod *inflated = mono_class_inflate_generic_method_full_checked (gclass->vtable [i], klass, mono_class_get_context (klass), &error);
				if (!mono_error_ok (&error)) {
				...
				tmp [i] = inflated;
				tmp [i]->slot = gclass->vtable [i]->slot;
			}
		mono_memory_barrier ();
		klass->vtable = tmp;

		/* Have to set method->slot for abstract virtual methods */
		if (klass->methods && gclass->methods) {
			for (i = 0; i < klass->method.count; ++i)
				if (klass->methods [i]->slot == -1)
					klass->methods [i]->slot = gclass->methods [i]->slot;
		}
	...
	if (klass->parent && klass->parent->vtable_size) {
		...
		// have different names in the iterface and the klass, because for child
	...
	for (i = 0; i < klass->interface_offsets_count; i++) {
		...
		for (im_index = 0; im_index < ic->method.count; im_index++) {
			...
			MonoMethod *override_im = (MonoMethod *)((override_map != NULL) ? g_hash_table_lookup (override_map, im) : NULL);
			...
			if (override_im == NULL) {
				...
				// First look for a suitable method among the klass methods
	...
	// If the klass is not abstract, check that all its interface slots are full.
	...
	if (! (klass->flags & TYPE_ATTRIBUTE_ABSTRACT)) {
		for (i = 0; i < klass->interface_offsets_count; i++) {
			...
			for (im_index = 0; im_index < ic->method.count; im_index++) {
				...
				TRACE_INTERFACE_VTABLE (printf ("      [klass is not abstract, checking slot %d for interface '%s'.'%s', method %s, slot check is %d]\n",
	...
	for (l = virt_methods; l; l = l->next) {
		 ...
		 * base klass for a method to override.
		 */
		if (!(cm->flags & METHOD_ATTRIBUTE_NEW_SLOT)) {
			int slot = -1;
	...
	if (klass->generic_class) {
		...
		klass->vtable_size = MAX (gclass->vtable_size, cur_slot);
	} else {
...
mono_method_get_vtable_slot (MonoMethod *method)
{
	if (method->slot == -1) {
		...
		if (method->slot == -1) {
			...
			gclass = method->klass->generic_class->container_class;
			...
			g_assert (gclass->methods);
			method->slot = gclass->methods [i]->slot;
		}
...
/**
 * mono_class_init:
 * @klass: the klass to initialize
 *
 *   Compute the instance_size, klass_size and other infos that cannot be 
...
mono_class_init (MonoClass *klass)
{
	...
	if (klass->generic_class && !klass->generic_class->is_dynamic) {
		...
		klass->method = gclass->method;
		klass->field = gclass->field;
	...
	if (has_cached_info) {
		...
		klass->ghcimpl = gclass->ghcimpl;
		klass->has_cctor = gclass->has_cctor;
...
mono_class_has_finalizer (MonoClass *klass)
{
	...
	if (!(MONO_CLASS_IS_INTERFACE (klass) || klass->valuetype)) {
		...
		if (klass->rank == 1 && klass->byval_arg.type == MONO_TYPE_SZARRAY) {
			...
			has_finalize = mono_class_has_finalizer (gclass);
...
mono_class_setup_mono_type (MonoClass *klass)
{
	...
	if (klass->valuetype) {
		...
		klass->this_arg.type = klass->byval_arg.type = (MonoTypeEnum)t;
	}
...
mono_class_setup_parent (MonoClass *klass, MonoClass *parent)
{
	...
	if (!MONO_CLASS_IS_INTERFACE (klass)) {
		...
		if (system_namespace) {
#ifndef DISABLE_REMOTING
			if (*klass->name == 'M' && !strcmp (klass->name, "MarshalByRefObject"))
				klass->marshalbyref = 1;

			if (*klass->name == 'C' && !strcmp (klass->name, "ContextBoundObject")) 
				klass->contextbound  = 1;
#endif
			if (*klass->name == 'D' && !strcmp (klass->name, "Delegate")) 
				klass->delegate  = 1;
		}
 ...
 * This currently sets two fields in @klass:
 *  - idepth: distance between @klass and System.Object in the type
 *    hierarchy + 1
 *  - supertypes: array of classes: each element has a klass in the hierarchy
 *    starting from @klass up to System.Object
 * 
...
mono_class_create_from_typedef (MonoImage *image, guint32 type_token, MonoError *error)
{
	 ...
	 * We must do this after the klass has been constructed to make certain recursive scenarios
	 * work.
	 */
...
make_generic_param_class (MonoGenericParam *param, MonoImage *image, gboolean is_mvar, MonoGenericParamInfo *pinfo)
{
	...
	if (pinfo) {
		...
		char *name = (char *)mono_image_alloc0 (image, 16);
		sprintf (name, "%d", n);
		klass->name = name;
	}
	...
	if (count - pos > 0) {
		...
		klass->interfaces = (MonoClass **)mono_image_alloc0 (image, sizeof (MonoClass *) * (count - pos));
	...
	klass->min_align = 1;
	/*
	 * This makes sure the the value size of this klass is equal to the size of the types the gparam is
...
set_anon_gparam_class (MonoGenericParam *param, gboolean is_mvar, MonoClass *klass)
{
	...
	if (param->gshared_constraint) {
		...
		if (is_mvar) {
			/* Requires locking to avoid droping an already published klass */
...
mono_fnptr_class_get (MonoMethodSignature *sig)
{
	...
	result->name = "MonoFNPtrFakeklass";
 ...
 * @element_class: element klass 
 * @rank: the dimension of the array klass
 * @bounded: whenever the array has non-zero bounds
 *
 * Returns: a klass object describing the array with element type @element_type and 
 * dimension @rank. 
 */
MonoClass *
mono_bounded_array_class_get (MonoClass *eklass, guint32 rank, gboolean bounded)
{
	MonoImage *image;
	MonoClass *klass;
	...
	if (rank == 1 && !bounded) {
		...
		klass = (MonoClass *)g_hash_table_lookup (image->szarray_cache, eklass);
		...
		if ((rootlist = list = (GSList *)g_hash_table_lookup (image->array_cache, eklass))) {
	...
	klass->image = image;
	klass->name_space = eklass->name_space;
	nsize = strlen (eklass->name);
	name = (char *)g_malloc (nsize + 2 + rank + 1);
	memcpy (name, eklass->name, nsize);
	name [nsize] = '[';
	if (rank > 1)
	...
	if (eklass->byval_arg.type == MONO_TYPE_TYPEDBYREF || eklass->byval_arg.type == MONO_TYPE_VOID) {
		/*Arrays of those two types are invalid.*/
		mono_class_set_failure (klass, MONO_EXCEPTION_TYPE_LOAD, NULL);
	} else if (eklass->enumtype && !mono_class_enum_basetype (eklass)) {
		if (!eklass->ref_info_handle || eklass->wastypebuilder) {
			g_warning ("Only incomplete TypeBuilder objects are allowed to be an enum without base_type");
			g_assert (eklass->ref_info_handle && !eklass->wastypebuilder);
		}
		/* element_size -1 is ok as this is not an instantitable type*/
		klass->sizes.element_size = -1;
	} else
		klass->sizes.element_size = mono_class_array_element_size (eklass);

	mono_class_setup_supertypes (klass);

	if (eklass->generic_class)
		mono_class_init (eklass);
	if (!eklass->size_inited)
		mono_class_setup_fields (eklass);
	if (eklass->exception_type) /*FIXME we fail the array type, but we have to let other fields be set.*/
		mono_class_set_failure (klass, MONO_EXCEPTION_TYPE_LOAD, NULL);

	klass->has_references = MONO_TYPE_IS_REFERENCE (&eklass->byval_arg) || eklass->has_references? TRUE: FALSE;

	klass->rank = rank;
	
	if (eklass->enumtype)
		klass->cast_class = eklass->element_class;
	else
		klass->cast_class = eklass;
	...
	if ((rank > 1) || bounded) {
		...
		klass->byval_arg.data.klass = eklass;
	}
	...
	klass->generic_container = eklass->generic_container;

	if (rank == 1 && !bounded) {
		MonoClass *prev_class;

		mono_mutex_lock (&image->szarray_cache_lock);
		prev_class = (MonoClass *)g_hash_table_lookup (image->szarray_cache, eklass);
		if (prev_class)
			/* Someone got in before us */
			klass = prev_class;
		else
			g_hash_table_insert (image->szarray_cache, eklass, klass);
		mono_mutex_unlock (&image->szarray_cache_lock);
	} else {
		list = g_slist_append (rootlist, klass);
		g_hash_table_insert (image->array_cache, eklass, list);
	}
 ...
 */
MonoClass *
mono_array_class_get (MonoClass *eklass, guint32 rank)
{
	return mono_bounded_array_class_get (eklass, rank, FALSE);
}
 ...
 * @klass: a klass 
 * 
 * Returns: the size of the static klass data
 */
gint32
 ...
 * @klass: the klass to lookup the field.
 ...
 * @name: the field name
 *
 * Search the klass @klass and it's parents for a field with the name @name.
 * 
 ...
 *
 * Search the klass @klass and it's parents for a field with the name @name and type @type.
 *
...
mono_class_get_field_default_value (MonoClassField *field, MonoTypeEnum *def_type)
{
	...
	if (!klass->ext->field_def_values [field_index].data) {
		...
		klass->ext->field_def_values [field_index].data = (const char *)(gpointer)mono_metadata_blob_heap (field->parent->image, constant_cols [MONO_CONSTANT_VALUE]);
	}
...
mono_class_get_property_default_value (MonoProperty *property, MonoTypeEnum *def_type)
{
	...
	*def_type = (MonoTypeEnum)constant_cols [MONO_CONSTANT_TYPE];
	return (const char *)(gpointer)mono_metadata_blob_heap (klass->image, constant_cols [MONO_CONSTANT_VALUE]);
}
...
/**
 * mono_class_get_full:
 * @image: the image where the klass resides
 * @type_token: the token for the klass
 * @context: the generic context used to evaluate generic instantiations in
...
mono_type_get_checked (MonoImage *image, guint32 type_token, MonoGenericContext *context, MonoError *error)
{
	...
	if (inflated) {
		 ...
		 * A MonoClass::byval_arg of a generic type definion has type KLASS.
 ...
 *
 *  Initializes the klass name cache stored in image->name_cache.
 *
 ...
 * For example use "Foo/Bar" to reference the klass Bar that is nested
 * inside Foo, like this: "klass Foo { klass Bar {} }".
 */
MonoClass *
 ...
 * @klass: klass to probe if it is a subklass of another one
 * @klassc: the klass we suspect is the base klass
 * @check_interfaces: whether we should perform interface checks
 *
 * This method determines whether @klass is a subklass of @klassc.
 *
 ...
 * then it returns true if the @klass is a subklass of @klassc.
 *
...
/*FIXME test for interfaces with variant generic arguments*/
	...
	/* 
	 * MS.NET thinks interfaces are a subklass of Object, so we think it as
	 * well.
	 */
 ...
 * @klass: the klass to be assigned to
 * @oklass: the source klass
 * 
...
{
	int j;
	MonoType **class_argv, **oclass_argv;
	MonoClass *class_gtd = mono_class_get_generic_type_definition (klass);
	MonoGenericContainer *container = class_gtd->generic_container;

	if (klass == oklass)
		return TRUE;

	/*Viable candidates are instances of the same generic interface*/
	if (mono_class_get_generic_type_definition (oklass) != class_gtd || oklass == class_gtd)
		return FALSE;

	class_argv = &klass->generic_class->context.class_inst->type_argv [0];
	oclass_argv = &oklass->generic_class->context.class_inst->type_argv [0];

	for (j = 0; j < container->type_argc; ++j) {
		MonoClass *param1_class = mono_class_from_mono_type (class_argv [j]);
		MonoClass *param2_class = mono_class_from_mono_type (oclass_argv [j]);
...
mono_class_is_assignable_from (MonoClass *klass, MonoClass *oklass)
{
	...
	if (MONO_CLASS_IS_INTERFACE (klass)) {
		...
		eklass = klass->cast_class;
		eoklass = oklass->cast_class;
		...
		if (eoklass->valuetype) {
			if ((eklass == mono_defaults.enum_class) || 
				(eklass == mono_defaults.enum_class->parent) ||
				(eklass == mono_defaults.object_class))
				return FALSE;
		}
...
mono_class_is_assignable_from_slow (MonoClass *target, MonoClass *candidate)
{
	...
	if (target->rank) {
		...
		eklass = target->cast_class;
		eoklass = candidate->cast_class;
...
mono_class_get_cctor (MonoClass *klass)
{
	...
	if (mono_class_get_cached_class_info (klass, &cached_info)) {
		...
		if (!mono_error_ok (&error))
			g_error ("Could not lookup klass cctor from cached metadata due to %s", mono_error_get_message (&error));
		return result;
	}
 ...
 *
 * Determines whenever the klass has a static constructor and whenever it
 ...
 * @ac: pointer to a #MonoArrayklass
 *
 ...
 *
 * Returns: the element klass of an array or an enumeration.
 */
MonoClass*
 ...
 * @klass: the MonoClass to act on
 *
 * Returns: the parent klass for this klass.
 */
MonoClass*
 ...
 * @klass: the MonoClass to act on
 *
 * Returns: the namespace of the klass.
 */
const char*
 ...
 * @klass: the MonoClass to act on
 *
 * This method returns the internal Type representation for the klass.
 *
 * Returns: the MonoType from the klass.
 */
MonoType*
 ...
 * @klass: the MonoClass to act on
 *
 * This method returns type token for the klass.
 *
 * Returns: the type token for the klass.
 */
guint32
 ...
 * @klass: the MonoClass to act on
 *
 * Returns: the number of static and instance fields in the klass.
 */
int
 ...
 * @klass: the MonoClass to act on
 *
 * Returns: the number of methods in the klass.
 */
int
 ...
 * @klass: the MonoClass to act on
 *
 * Returns: the number of properties in the klass.
 */
int
 ...
 * @klass: the MonoClass to act on
 *
 * Returns: the number of events in the klass.
 */
int
 ...
 * @klass: the MonoClass to act on
 *
 * This routine is an iterator routine for retrieving the fields in a klass.
 *
 ...
 * @klass: the MonoClass to act on
 *
 * This routine is an iterator routine for retrieving the properties in a klass.
 *
 * You must pass a gpointer that points to zero and is treated as an opaque handle to
 * iterate over all of the elements.  When no more values are
 * available, the return value is NULL.
 ...
 * @klass: the MonoClass to act on
 *
 * This routine is an iterator routine for retrieving the interfaces implemented by this klass.
 *
 * You must pass a gpointer that points to zero and is treated as an opaque handle to
 * iterate over all of the elements.  When no more values are
 * available, the return value is NULL.
...
setup_nested_types (MonoClass *klass)
{
	...
	while (i) {
		...
		nklass = mono_class_create_from_typedef (klass->image, MONO_TOKEN_TYPE_DEF | cols [MONO_NESTED_CLASS_NESTED], &error);
		if (!mono_error_ok (&error)) {
		...
		classes = g_list_prepend (classes, nklass);
 ...
 * @klass: the MonoClass to act on
 *
 * This routine is an iterator routine for retrieving the nested types of a klass.
...
mono_class_get_exception_for_failure (MonoClass *klass)
{
	...
	default: {
		...
		/* TODO - handle other klass related failures */
		return NULL;
	}
	}
}
 ...
 * is subklass of Subtype. For example:
 * klass A { protected int x; }
 * klass B : A {
 * 	void valid_access () {
 * 		B b;
 * 		b.x = 0;
...
can_access_internals (MonoAssembly *accessing, MonoAssembly* accessed)
{
	...
	for (tmp = accessed->friend_assembly_names; tmp; tmp = tmp->next) {
		MonoAssemblyName *friend_assembly = (MonoAssemblyName *)tmp->data;
		/* Be conservative with checks */
		if (!friend_assembly->name)
			continue;
		if (strcmp (accessing->aname.name, friend_assembly->name))
			continue;
		if (friend_assembly->public_key_token [0]) {
			if (!accessing->aname.public_key_token [0])
				continue;
			if (!mono_public_tokens_are_equal (friend_assembly->public_key_token, accessing->aname.public_key_token))
				continue;
		}
		return TRUE;
	}
 ...
 * @source_class: The source klass 
 * @target_class: The accessed klass
 * 
 ...
 * @klass: An enum klass to be validated
 *
 * This method verify the required properties an enum should have.
 *  
 * Returns: TRUE if the informed enum klass is valid 
 *
...
gboolean
mono_generic_class_is_generic_type_definition (MonoGenericClass *gclass)
{
	return gclass->context.class_inst == gclass->container_class->generic_container->context.class_inst;
}
...
mono_class_setup_interfaces (MonoClass *klass, MonoError *error)
{
	...
	if (klass->rank == 1 && klass->byval_arg.type != MONO_TYPE_ARRAY) {
		...
		mono_class_setup_interfaces (gclass, error);
		if (!mono_error_ok (error)) {
			mono_class_set_failure (klass, MONO_EXCEPTION_TYPE_LOAD, g_strdup ("Could not setup the interfaces"));
			return;
		}

		interface_count = gclass->interface_count;
		interfaces = mono_class_new0 (klass, MonoClass *, interface_count);
		for (i = 0; i < interface_count; i++) {
			interfaces [i] = mono_class_inflate_generic_class_checked (gclass->interfaces [i], mono_generic_class_get_context (klass->generic_class), error);
			if (!mono_error_ok (error)) {
				mono_class_set_failure (klass, MONO_EXCEPTION_TYPE_LOAD, g_strdup ("Could not setup the interfaces"));
				return;
			}
