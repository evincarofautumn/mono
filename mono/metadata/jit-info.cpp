...
jit_info_table_check (MonoJitInfoTable *table)
{
	...
	for (i = 0; i < table->num_chunks; ++i) {
		...
		for (j = 0; j < chunk->num_elements; ++j) {
			MonoJitInfo *current = chunk->data [j];
			MonoJitInfo *next;

			g_assert ((gint8*)current->code_start + current->code_size <= chunk->last_code_end);
			...
			g_assert ((gint8*)current->code_start + current->code_size <= (gint8*)next->code_start + next->code_size);
		}
	}
}
