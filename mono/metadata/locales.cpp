...
{
	return (const CultureInfoEntry *)mono_binary_search (&lcid, culture_entries, NUM_CULTURE_ENTRIES, sizeof (CultureInfoEntry), culture_lcid_locator);
}
...
MonoString *ves_icall_System_String_InternalReplace_Str_Comp (MonoString *this_obj, MonoString *find, MonoString *replace, MonoCompareInfo *comp)
{
	/* Do a normal ascii string compare and replace, as we only
	 * know the invariant locale if we dont have ICU
	 */
	return(string_invariant_replace (this_obj, find, replace));
}
