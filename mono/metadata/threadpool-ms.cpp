...
worker_park (void)
{
	...
	if (!mono_runtime_is_shutting_down ()) {
		...
		if (mono_cond_timedwait_ms (&threadpool->parked_threads_cond, &threadpool->active_threads_lock, rand_next ((gpointer *)rand_handle, 5 * 1000, 60 * 1000)) != 0)
			timeout = TRUE;
...
worker_try_create (void)
{
	...
	if ((thread = mono_thread_create_internal (mono_get_root_domain (), (gpointer)worker_thread, NULL, TRUE, 0)) != NULL) {
	...
	for (;;) {
		...
		case MONITOR_STATUS_NOT_RUNNING:
			...
			if (InterlockedCompareExchange (&monitor_status, MONITOR_STATUS_REQUESTED, MONITOR_STATUS_NOT_RUNNING) == MONITOR_STATUS_NOT_RUNNING) {
				if (!mono_thread_create_internal (mono_get_root_domain (), (gpointer)monitor_thread, NULL, TRUE, SMALL_STACK))
