...
mono_image_insert_string (MonoReflectionModuleBuilder *module, MonoString *str)
{
...
#if G_BYTE_ORDER != G_LITTLE_ENDIAN
	{
		char *swapped = g_malloc (2 * mono_string_length_fast (str, TRUE));
		const char *p = (const char*)mono_string_to_utf16 (str);
		swap_with_size (swapped, p, 2, mono_string_length_fast (str, TRUE));
...
#else
	{
		...
		mono_image_add_stream_data (&assembly->us, (const char *)chars, mono_string_length_fast (str, TRUE) * sizeof (gunichar2));
		g_free (chars);
	}
#endif
