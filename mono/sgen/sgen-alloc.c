 ...
 */
void
sgen_gc_stick_region_if_necessary (gpointer src, gpointer dst) {
// #ifndef HAVE_KW_THREAD
	SgenThreadInfo *__thread_info__ = mono_thread_info_current ();
// #endif
	sgen_gc_lock ();
	...
	gboolean old_frame_to_new_frame = sgen_client_ptr_on_stack (dst);
...
void
sgen_gc_region_exit (gpointer ret_)
{
	MonoObject *ret = (MonoObject *)ret_;
// #ifndef HAVE_KW_THREAD
	SgenThreadInfo *__thread_info__ = mono_thread_info_current ();
// #endif
	char *region;
	...
	char *next;
	if (ret)
		sgen_gc_stick_region_if_necessary (ret, NULL);
	sgen_gc_lock ();
