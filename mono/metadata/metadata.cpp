									...
									gboolean transient, const char *ptr, const char **rptr)
{
	...
	array = (MonoArrayType *)(transient ? g_malloc0 (sizeof (MonoArrayType)) : mono_image_alloc0 (m, sizeof (MonoArrayType)));
	...
	if (array->numsizes)
		array->sizes = (int *)(transient ? g_malloc0 (sizeof (int) * array->numsizes) : mono_image_alloc0 (m, sizeof (int) * array->numsizes));
	for (i = 0; i < array->numsizes; ++i)
		array->sizes [i] = mono_metadata_decode_value (ptr, &ptr);

	array->numlobounds = mono_metadata_decode_value (ptr, &ptr);
	if (array->numlobounds)
		array->lobounds = (int *)(transient ? g_malloc0 (sizeof (int) * array->numlobounds) : mono_image_alloc0 (m, sizeof (int) * array->numlobounds));
								   ...
								   short opt_attrs, gboolean transient, const char *ptr, const char **rptr)
{
	...
	if (count) {
		...
		type = (MonoType *)(transient ? g_malloc0 (size) : mono_image_alloc0 (m, size));
		type->num_mods = count;
		if (count > 64)
	...
	if (type == &stype) {
		type = (MonoType *)(transient ? g_malloc (MONO_SIZEOF_TYPE) : mono_image_alloc (m, MONO_SIZEOF_TYPE));
							 ...
							 gboolean transient, const char *ptr, const char **rptr)
{
	...
	case MONO_TYPE_CLASS: {
		...
		if (!compare_type_literals (klass->byval_arg.type, type->type))
			return FALSE;
		break;
	}
	case MONO_TYPE_SZARRAY: {
...
mono_metadata_type_dup (MonoImage *image, const MonoType *o)
{
	...
	r = (MonoType *)(image ? mono_image_alloc0 (image, sizeof_o) : g_malloc (sizeof_o));
